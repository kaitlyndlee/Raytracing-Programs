...
void ppm_ReadInP3 (FILE *inFile, PPMFormat inData) {
  ...
  for (int y = 0; y < inData.height; y++) {
    for (int x = 0; x < inData.width; x++) {
      for (int p = 0; p < 3; p++){
        ppm_AtEndOfFile(inFile, (char *) "The input file is too short based on the given height and width.");
 ...
 */
__device__ float ppm_clamp(float value, float lower_bound, float upper_bound) {
