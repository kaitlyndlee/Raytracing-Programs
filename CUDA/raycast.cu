...
int main(int argc, char **argv) {
  ...
  raytrace_engine<<<num_blocks, threads_per_block>>>(dev_pixel_height, dev_pixel_width, json_struct,
                                                     photo_data);
 ...
 */
__global__ void raytrace_engine(float *pixel_height,
                                float *pixel_width,
                                json_data_t *json_struct,
                                PPMFormat *photo_data) {
  ...
  if (tid == 0) {
    ...
    if (tid < json_data.num_shapes + 4) {
      ...
      json_data.lights_list[tid - json_data.num_shapes - 4] =
          json_struct->lights_list[tid - json_data.num_shapes - 4];
    }
  }
  __syncthreads();
  ...
  if (x < photo_data->width && y < photo_data->height) {
    ...
    ray_direction[0] =
        view_plane_center[0] - (json_data.camera_width / 2.0) + *pixel_width * (x + 0.5);
    ray_direction[1] =
        view_plane_center[1] + (json_data.camera_height / 2.0) - *pixel_height * (y + 0.5);
 ...
 */
__device__ void iterative_shoot(json_data_t *json_struct,
                                float *ray_origin,
                                float *ray_direction,
                                float *out_color,
                                int skip_index) {
  ...
  nearest_object_index =
      find_nearest_object(json_struct, &nearest_object, ray_origin, ray_direction, intersection,
                          normal, skip_index, false);
  ...
  while (iter < MAX_ITER) {
    ...
    if (next_nearest_object->reflectivity > 0) {
      ...
      next_nearest_object_index = find_nearest_object(
          json_struct, &next_nearest_object, next_ray_origin, next_ray_direction, next_intersecion,
          next_normal, next_skip_index, false);
      if (next_nearest_object != NULL) {
        calc_color(json_struct, color, next_nearest_object_index, next_intersecion, next_normal,
                   next_ray_direction);
 ...
 * @param ray_direction - ray direction vector
 */
__device__ void calc_color(json_data_t *json_struct,
                           float *out_color,
                           int object_index,
                           float *intersection,
                           float *normal,
  ...
  if (opacity > 0.0) {
    ...
    for (int lights = 0; lights < json_struct->num_lights; lights++) {
      ...
      find_nearest_object(json_struct, &obj_intersection, intersection, light_direction, NULL, NULL,
                          object_index, true);
 ...
 */
__device__ int find_nearest_object(json_data_t *json_struct,
                                   shape_t **nearest_object,
                                   float *ray_origin,
                                   float *ray_direction,
                                   float *intersection,
                                   float *normal,
  ...
  for (int count = 0; count < json_struct->num_shapes; count++) {
    ...
    if (json_struct->shapes_list[count].type == SPHERE) {
      if (ray_intersect_sphere(ray_origin, ray_direction, &json_struct->shapes_list[count],
                               &distance)) {
      ...
      if (ray_intersect_plane(ray_origin, ray_direction, &json_struct->shapes_list[count],
                              &distance)) {
        if (nearest_distance > distance && distance > 0) {
          nearest_distance = distance;
          *nearest_object = &json_struct->shapes_list[count];
      ...
      if (ray_intersect_quadric(ray_origin, ray_direction, &json_struct->shapes_list[count],
                                &distance)) {
 ...
 */
__device__ bool
ray_intersect_quadric(float *ray_o, float *ray_d, shape_t *quadric, float *distance) {
  ...
  float b_q = 2.0 * quadric->a * ray_o[0] * ray_d[0] + 2.0 * quadric->b * ray_o[1] * ray_d[1] +
              2.0 * quadric->c * ray_o[2] * ray_d[2] +
              quadric->d * (ray_o[0] * ray_d[1] + ray_o[1] * ray_d[0]) +
              quadric->e * (ray_o[0] * ray_d[2] + ray_o[2] * ray_d[0]) +
              quadric->f * (ray_o[1] * ray_d[2] + ray_o[2] * ray_d[1]) + quadric->g * ray_d[0] +
              quadric->h * ray_d[1] + quadric->i * ray_d[2];

  float c_q = quadric->a * powf(ray_o[0], 2) + quadric->b * powf(ray_o[1], 2) +
              quadric->c * powf(ray_o[2], 2) + quadric->d * ray_o[0] * ray_o[1] +
              quadric->e * ray_o[0] * ray_o[2] + quadric->f * ray_o[1] * ray_o[2] +
              quadric->g * ray_o[0] + quadric->h * ray_o[1] + quadric->i * ray_o[2] + quadric->j;
...
__device__ float radial_attenuation(light_t *light, float distance) {
  return 1.0 / (light->radial_coef[0] + light->radial_coef[1] * distance +
                light->radial_coef[2] * powf(distance, 2));
}
 ...
 */
__device__ void diffuse_light(float *return_color,
                              light_t *light,
                              float *object_color,
 ...
 */
__device__ void specular_light(float *return_color,
                               light_t *light,
                               float *object_color,
                               float *surface_normal,
                               float *light_vector,
