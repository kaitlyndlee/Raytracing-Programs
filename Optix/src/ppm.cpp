...
/**
 * Checks to see if the file is at its end. If it is at the end the function,
 * will call ppm_Fail with the given error message.
 *
 * @param inFile The input file
 * @param errorMessage The error message to send to the screen if the file is at
 * the end of the file
 */
...
/**
 * Reads in a P3 PPM file. Takes all the image data and stores it in the
 * PPMFormat data structure's pixmap. This fucntion checks to see if the image
 * is the correct width and height as stated in the PPM file header.
 *
 * @param inFile The input file
...
void ppm_ReadInP3(FILE *inFile, PPMFormat inData) {
  ...
  for (int y = 0; y < inData.height; y++) {
    for (int x = 0; x < inData.width; x++) {
      for (int p = 0; p < 3; p++) {
        ppm_AtEndOfFile(inFile, (char *) "The input file is too short based on "
        ...
        if (buffer > inData.maxColor) {
          ppm_Fail((char *) "Pixel exceeds max color.");
        }
        pixel[p] = num;
      }
      pixel[3] = 255;
  ...
  if (getc(inFile) != EOF) {
    ppm_Fail((char *) "The input file is too long for the given width and height.");
  }
}

/**
 * Reads in a P6 PPM file. Takes all the image data and stores it in the
 * PPMFormat data structure's pixmap. This fucntion checks to see if the image
 * is the correct width and height as stated in the PPM file header.
 *
 * @param inFile The input file
...
void ppm_ReadInP6(FILE *inFile, PPMFormat inData) {
  ...
  for (int y = 0; y < inData.height; y++) {
    for (int x = 0; x < inData.width; x++) {
      ...
      if (entriesRead < 1) {
        ppm_Fail((char *) "The input file is too short based on the given "
                          "height and width.");
      }

      if (buffer[0] > inData.maxColor || buffer[1] > inData.maxColor ||
          buffer[2] > inData.maxColor) {
        ppm_Fail((char *) "Pixel exceeds max color.");
      }
...
/**
 * Reads in a P7 PPM file. Takes all the image data and stores it in the
 * PPMFormat data structure's pixmap. This fucntion checks to see if the image
 * is the correct width and height as stated in the PPM file header.
 *
 * @param inFile The input file
...
/**
 * Prints the pixmap from the PPMFormat data structure into the output file.
 * This fucntion will print in P3 PPM format, so it will print out in ASCII.
 *
 * @param inData The data structure holding the pixmap and PPM values
...
void ppm_WriteOutP3(PPMFormat inData, FILE *outFile) {
  ...
  for (int y = 0; y < inData.height; y++) {
    for (int x = 0; x < inData.width; x++) {
      pixel = &inData.pixmap[3 * ((y * inData.width) + x)];
...
/**
 * Prints the pixmap from the PPMFormat data structure into the output file.
 * This fucntion will print in P6 PPM format, so it will print out in raw
 * binary.
 *
 * @param inData The data structure holding the pixmap and PPM values
 * @param outFile The output file
...
/**
 * Prints the pixmap from the PPMFormat data structure into the output file.
 * This fucntion will print in P7 PPM format, so it will print out in raw
 * binary.
 *
 * @param inData The data structure holding the pixmap and PPM values
 * @param outFile The output file
...
/**
 * Reads a line from the header, skipping comments that come before and after
 * any tokens.
 *
...
/**
 * Takes in a buffer string and stores the tokens in two output strings,
 * skipping whitespace. Used to grab the header name and value, e.g., 'HEADER'
 * and '350'.
 *
...
uint8_t *ppm_RemoveAlphaChannel(PPMFormat inData) {
  uint8_t *tempArray = (uint8_t *) malloc(inData.width * inData.height * 3);
