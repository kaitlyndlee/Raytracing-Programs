...
void parse_json(FILE *json, json_data_t *json_data) {
  ...
  while (true) {
    ...
    if (strcmp(buffer, "camera") == 0) {
      ...
      add_new_sphere(json_data->shapes_list, json_data->num_shapes, diffuse, specular, pos, radius,
      ...
      add_new_plane(json_data->shapes_list, json_data->num_shapes, diffuse, specular, pos, norm,
                    reflectivity);
      json_data->num_shapes += 1;
    }
      ...
      add_new_quadric(json_data->shapes_list, json_data->num_shapes, diffuse, specular,
                      quadric_params[0], quadric_params[1], quadric_params[2], quadric_params[3],
                      quadric_params[4], quadric_params[5], quadric_params[6], quadric_params[7],
      ...
      if (isSpotlight) {
        add_new_spot_light(json_data->lights_list, json_data->num_lights, color, pos, theta, a0,
                           direction, radial_coef);
      }
      else {
        add_new_point_light(json_data->lights_list, json_data->num_lights, color, pos, radial_coef);
      }
